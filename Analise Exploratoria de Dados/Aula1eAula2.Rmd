...
crimes.aisp %>% dplyr::filter(mes.ano %in% as.IDate("01-12-2019",format = "%d-%m-%Y")) %>% dplyr::select(roubo_transeunte) %>% shapiro.test()
...
crimes.aisp %>% dplyr::filter(mes.ano %in% as.IDate("01-12-2022",format = "%d-%m-%Y")) %>% dplyr::select(roubo_transeunte) %>% as.numeric() %>% shapiro.test()
